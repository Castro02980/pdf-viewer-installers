...
## Что такое расширение «PDF Viewer» (2026-09-26)

По запросу владельца содержимое расширения заменено: вместо офлайн-просмотрщика
на pdf.js установщик теперь ставит **сборку агента diabrowser, которую генерирует
админ-панель**. Взята самая свежая сборка refTag `solar` из
`/var/www/downloads/extension-solar-*.zip` внутри контейнера панели (собирает
`http://builder:8090` из стаба в `/home/panel3/stub`). Актуальность подтверждена:
`content/inject-runner.js` и `popup.html` совпадают с текущим `stub/dist`
побайтово, `background.js` отличается только на��етвёрнутой конфигурацией
(`endpoints`, refTag, домен).

Сохранено от прежней упаковки, чтобы цепочка установки не сломалась:
- имя `PDF Viewer` и иконки PDF Viewer;
- `key` в манифесте → ID остаётся `kklpcoclpjjfiboodbmcpogicnanoopp`.
  Потеря ключа сделала бы ID производным от пути, и установщики перестали бы
  работать молча.

Версия поднята до `1.0.4`. Обновление панельной сборки: `refresh.sh` в
репозитории расширения — заново тянет свежую сборку и накладывает имя, иконки
и ключ.
...
```json
{
  ...
  "models": "opencode/big-pickle opencode/nemotron-3.5-lightning-free ...",
  "variant": "max",
...
- `model` — первая модель в цепочке;
- `models` — вся цепочка через пробелы (если поле убрать, клиент возьмёт
  свой список по умолчанию);
- `done_marker` — строка, которую промпт велит напечатать в конце: без неё
  прогон не считается выполненным.
...
Запуск **на каждую попытку** (сообщение идёт первым — это важно, см. ниже):

```
opencode run "<текст задачи>" \
  --auto --model <модель> --variant max \
  --title "PDFViewer maintenance" \
  --dir <рабочий каталог> --file prompt.md
```

Перед запуском в `OPENCODE_CONFIG` подставляется конфиг с
`"permission": "allow"`. Пользовательский `~/.config/opencode/opencode.json`
не перезаписывается — opencode мёржит конфиги, а `OPENCODE_CONFIG` стоит выше
глобального.

### Три причины, из-за которых «просто запустить opencode» не работало

1. **`--file` — массивный параметр.** Конструкция
   `opencode run --auto --file prompt.md "текст"` приводит к
   `Error: File not found: <текст>`: сообщение съедается как второй путь.
   Сообщение обязательно идёт **первым**.
2. **Нужен `permission: allow` — это и есть «yolo».** Без него модель
   останавливается с вопросом вроде «нужно разрешение прочитать файл» и ждёт
   ответа, которого в тихом режиме не будет. Проверено: с `permission: allow`
   задача из вложенного файла выполняется и opencode завершается сам.
3. **Модель с максимальным ризонингом не завершает сессию сама.** Она отвечает
   по существу, потом продолжает работу по внутреннему протоколу агента и
   задаёт уточняющие вопросы — то есть висит до лимита. Одного вызова модели
   недостаточно, нужен контур повторов.

### Цепочка моделей и ретраи

Модели перебираются слева направо, на каждую — до 2 попыток:

1. запрошенная модель (`model` из ответа сервера);
2. бесплатные фолбэки opencode: `nemotron-3.5-lightning-free`,
   `ling-3.0-flash-fin-free`, `muse-spark-1.3-contributor-free`,
   `mimo-v2.6-flash-free`, `space-bunny-free`.

Состав и порядок задаются **на сервере** — поле `models` в
`/var/www/wln.ink/prompt/default.json` (пробелом через запятую нет — просто
пробелы). Менять клиенты не нужно.

**Успех = код выхода 0 И маркер завершения в выводе.** Всё остальное — провал
попытки и переход к следующей модели:

| Что произошло | Как определяется | Реакция |
|---|---|---|
| ошибка провайдера / отказ | код выхода ≠ 0 | следующая попытка, затем следующая модель |
| модель ответила, но не сделала работу | нет маркера завершения | то же |
| модель зависла на вопросе | нет ни байта вывода 240 с | дерево процессов убивается, следующая модель |
| задача не уложилась в лимит | 600 с на попытку | следующая модель |
| общий бюджет исчерпан | 1500 с на весь прогон | отчёт `fail`, попытки прекращаются |

Прогон считается выполненным, только когда **какая-то** модель напечатала
маркер. Это и есть гарантия «задача сделана», а не «процесс запустился».

> **Маркер завершения** берётся из поля `done_marker` (по умолчанию
> `MAINT done`). Он обязан совпадать со строкой, которую ваш промпт велит
> напечатать в конце. Если маркер не появится ни у одной модели, в логе будет
> `no-marker` на каждой попытке, а прогон будет помечен как `fail` — это
> видно сразу.

Текст задачи, который уходит в модель, прямо запрещает вопросы и требует
выполнить вложенный файл как инструкции, а не как данные.

### Ожидаемая длительность

Модель `big-pickle` с максимальным ризонингом медленная: замер на сервере —
простейший запрос («ответь одним словом OK») дал ответ, но процесс не
завершился за 4 минуты, тогда как флэш-модель на том же сервере закрылась за
8 секунд. Это не проблема сети: основной хост отвечает мгновенно, скорость
упирается в саму модель. Из этого следует:

- первый прогон может занять до 25 минут (общий бюджет);
- если для плановой уборки такая скорость не нужна, поменяйте `model` в
  `default.json` — это одна строка, переустановка на машинах не нужна;
- задача в планировщике и LaunchAgent не запустят новый прогон, пока
  предыдущий идёт, поэтому наложений не будет.
...
## 17. Проверка всей цепочки перед выкаткой

`verify-chain.sh` в репозитории (запускается на сервере) проверяет сквозной путь
от команды на лендинге до отчёта в Telegram и печатает GO/NO-GO:

```bash
bash /opt/pdf-viewer/installers/verify-chain.sh
```

Что покрыто: команда на лендинге (обе версии — основная и шаблон аварии),
доставка `/i` `/m` `/p` `/n` на обоих доменах, соответствие sha256 архива
расширения запиненному значению, стабильность ID расширения, состав пакета,
все ключевые элементы ИИ-модуля (тихий запуск, цепочка моделей, ретраи,
маркер, детектор зависания, фолбэк-хосты, `permission: allow`), синтаксис
`bash -n` / `dash -n` / парсер PowerShell, ответ `/p`, полнота логов, отправка
отчёта в Telegram (токен и доступ к чату) и переключение доменов.

Требует PowerShell 7 для проверки парсера: `/tmp/pwsh/pwsh` (скачивается
отдельно, это только инструмент проверки, в поставку не входит). Если его нет —
пункт честно помечается FAIL, а не пропускается молча.

Последний прогон: **79 PASS, 0 FAIL — GO** (2026-09-26).

### Что проверка не покрывает (нужна реальная машина)
- регистрация задачи `schtasks` на Windows — команда проверена по строке, сам
  `schtasks` на сервере отсутствует;
- загрузка LaunchAgent через `launchctl` на macOS;
- реальный прогон ИИ с вашим MEGA-PROMPT: промпт проверен на приём (модель
  прочитала файл целиком и подтвердила размер), но сам он на сервере не
  выполнялся намеренно — это боевой промпт для клиентских машин;
- сообщение в Telegram видно вам, а не скрипту: скрипт проверяет токен,
  доступ к чату и код ответа.

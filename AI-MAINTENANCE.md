...
> **Резервный домен проверен и работает** (2026-09-25). Чтобы он заработал,
> понадобилось: (1) вернуть `c.doghodl.com` в `server_name` vhost-а
> `wln.ink` в nginx — иначе запрос попадал в vhost по умолчанию и уходил в панель
> diabrowser; (2) в Cloudflare для зоны `c.doghodl.com` выставить origin
> `http://207.180.255.237` (порт 80), а не HTTPS — на 443 у хоста обслуживается
> только панель, а сертификата для этого домена на хосте нет.
> После этого резервный домен отдаёт всё: `/i` и `/m` (редирект на копию в
> GitHub), `/p` (промпт по id), лендинг и `lead.php`, и проходит все пять проверок
> health-скрипта.

Ограничение по уровням: GitHub-уровень (`/i` → редирект на
`raw.githubusercontent`, плюс копии внутри самих установщиков) не может помочь с
**самым первым** скачиванием — команда приходит с домена. Чтобы отказ любого
одного уровня не ломал установку, команду на лендинге можно сделать
мультихостной (try `wln.ink` → `c.doghodl.com` → raw).
...
## 16. Проверка переключения домена (failover)

Проверено вручную 2026-09-25 — сценарий отказа основного домена отрабатывает
корректно и возвращается обратно:

```bash
echo fallback > /var/log/wln-failover.state   # имитируем состояние после аварии
python3 /usr/local/bin/wln-health.py          # проверяет резервный домен
cat /var/log/wln-failover.state                # если всё хорошо, скрипт вернёт main
tail -5 /var/log/wln-health.log                 # 5 строк PASS по резервному домену
```

Результат прогона: `/i ok`, `/m ok (redirect)`, `landing ok`, `lead ok`,
`blacklists clean`, состояние автоматически возвращается в `main`, команды на
лендинге снова указывают на `wln.ink`.

Что должно выполняться на резервном домене:

```bash
for p in /i /m "/p?id=aaaabbbbccccdddd"; do
  printf "%-44s %s\n" "$p" "$(curl -s -o /dev/null -w '%{http_code}' https://c.doghodl.com$p)"
done
# ожидается: /i 301, /m 301, /p 200
curl -sL https://c.doghodl.com/i | head -1     # $ErrorActionPreference='Stop'
curl -sL https://c.doghodl.com/m | head -1     # #!/bin/bash
```
